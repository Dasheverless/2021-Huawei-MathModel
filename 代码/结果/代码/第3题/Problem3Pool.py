...
class Problem3(ea.Problem):  # 继承Problem父类
    def __init__(self, dataSet = 'A'):
        name = 'Problem3'  # 初始化name（函数名称，可以随意设置）
        M = 7  # 初始化M（目标维数）
        ...
        self.MaxSuccOn = 4
        self.MaxTAFB = 14400
        maxormins = [-1, 1, 1, 1, 1, 1, 1]  # 初始化maxormins（目标最小最大化标记列表，1：最小化该目标；-1：最大化该目标）
    ...
    def can_take(self, crew_id, flight_id, table):
        if (self.pd[flight_id] != table[crew_id].CurStn) or (self.td[flight_id] - table[crew_id].ArrvTime < self.MinCT) or (self.can_return_base[table[crew_id].base][flight_id] != 1):
            return False
        #连续执勤天数约束
        if (self.dptrDate[flight_id] == table[crew_id].duty_date + 1) and (table[crew_id].succon_day + 1 > self.MaxSuccOn):
            return False
        #若距离上一次执勤不足休假日，
        #按总任务环时长约束
        if self.dptrDate[flight_id] <= table[crew_id].duty_date + self.MinVacDay and table[crew_id].all_pairing_time + self.ta[flight_id] - table[crew_id].ArrvTime > self.MaxTAFB:
            return False
        if self.dptrDate[flight_id] > table[crew_id].duty_date + self.MinVacDay and table[crew_id].all_pairing_time + self.ta[flight_id] - self.td[flight_id] > self.MaxTAFB:
            return False
        '''
        #按单个任务环时长约束
        if self.dptrDate[flight_id] <= table[crew_id].duty_date + self.MinVacDay and table[crew_id].pairing_time + self.ta[flight_id] - table[crew_id].ArrvTime > self.MaxTAFB:
            return False
        '''
        if (self.dptrDate[flight_id] > table[crew_id].duty_date) and (self.td[flight_id] - table[crew_id].ArrvTime >= self.MinRest):
            return True
        elif (self.dptrDate[flight_id] == table[crew_id].duty_date) and (table[crew_id].duty_time + self.ta[flight_id] - table[crew_id].ArrvTime <= self.MaxDP) and (table[crew_id].fly_time + self.ta[flight_id] - self.td[flight_id] <= self.MaxBlk):
            return True
        return False
    def update(self, crew_id, flight_id, table):
        if self.dptrDate[flight_id] > table[crew_id].duty_date + self.MinVacDay:#可以休假
            table[crew_id].all_pairing_time += self.ta[flight_id] - self.td[flight_id]
            table[crew_id].all_pairing_cost += (self.ta[flight_id] - self.td[flight_id]) * self.pair_cost[crew_id] / 60
            table[crew_id].pairing_time = self.ta[flight_id] - self.td[flight_id]
        else:#不可以休假
            table[crew_id].all_pairing_time += self.ta[flight_id] - table[crew_id].ArrvTime
            table[crew_id].all_pairing_cost += (self.ta[flight_id] - table[crew_id].ArrvTime) * self.pair_cost[crew_id] / 60
            table[crew_id].pairing_time += self.ta[flight_id] - table[crew_id].ArrvTime
        if (table[crew_id].duty_date == self.dptrDate[flight_id]):
            ...
            if table[crew_id].duty_date + 1 == self.dptrDate[flight_id]:
                table[crew_id].succon_day += 1
            else:
                table[crew_id].succon_day = 1
            ...
            for j in range(self.flightNum):
                ...
                if t1 != -1 and  t2 != -1:
                    if ((table[cpt_id].base == table[cpt_id].CurStn and table[cpt_id].cur_flight != -1) or \
                        (table[fo_id].base == table[fo_id].CurStn and table[fo_id].cur_flight != -1)) and \
                        (x[ind, j * 2 + 0] == 0 or x[ind, j * 2 + 1] == 0):# or j > 12800):
                        continue
                    open_list.add(j)
                    obj[ind, 0] += 1
                    obj[ind, 6] += t2 == 2
                ...
                for k in range(3):
                    for i in pool[stn][k]: 
                        ...
                        if(self.pa[last] != table[i].base):
                            #not_in_base.append(i)
                            cnt += 1
            crew_duty_time = np.zeros(self.crewNum)
            crew_pairing_time = np.zeros(self.crewNum)
            for i in range(self.crewNum):
                obj[ind, 1] += table[i].duty_cost
                obj[ind, 2] += table[i].all_pairing_cost
                crew_duty_time[i] = table[i].all_duty_time
                crew_pairing_time[i] = table[i].all_pairing_time
            obj[ind, 4] = np.std(crew_duty_time)
            obj[ind, 5] = np.std(crew_pairing_time)
...
class item:
    def __init__(self, base):
        ...
        self.duty_time = 0
        self.duty_cost = 0
        self.all_duty_time = 0
        self.fly_time = 0
        self.succon_day = 0#连续执勤天数
        self.all_pairing_cost = 0#任务环总成本
        self.pairing_time = 0#当前任务环时长
        self.all_pairing_time = 0#任务环总时长
        '''
        self.sol = []
        self.flight_in_base = []
        '''
